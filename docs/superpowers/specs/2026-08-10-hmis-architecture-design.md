...
- **v4.8 2026-08-25** — the **STAGE-2 ACCELERATION** (owner ruling, in-conversation): new **§11.16-A — mini-OT day-care unit on the OPD floor** (gynecology + orthopedics; MTP and PCPNDT statutory machinery, consignment implants, CSSD-lite, two recovery bays, the overnight-conversion boundary rule), with inline `(v4.8)` amendments in §17 and §19. The two-track module order (14 procurement → 15 mini-OT → 16 pharmacy → 17 LIMS → 18 radiology ∥ 19 housekeeping/laundry/BMW, all after Plan 13) lives in the Phase-1 roadmap's stage-2 section, which is authoritative for sequencing.
...
### 11.16-A Mini-OT — day-care surgical unit on the OPD floor (v4.8, owner ruling 2026-08-25)

**Physical model (owner facts, 2026-08-25): one theatre beside the OPD floor serving gynecology and orthopedics day-care · two recovery bays · an adjacent autoclave/CSSD arrangement · ortho implants held as consignment stock.** Sequenced **ahead of the major-OT/ICU/IPD cluster** (stage-2 acceleration; order in the roadmap). This section instantiates existing law at one-theatre scale; it defines no new pattern except where marked.

**The unit is map 11's day-care cascade running §11.9's gates:** booking from the OPD consult's procedure-advice branch (procedure, side/site, surgeon, implant needs, day-care tariff quote + deposit per §11.11) → **case-selection criteria are governed definition data, Class B** — per-department procedure whitelist, ASA/age/comorbidity limits, escort-required flag; a case outside criteria routes to the major-OT pathway (referral out until that suite exists) → **mini-OT list published the previous evening** (§11.16's coordination-artifact rule: the list synchronizes CSSD set preparation, consignment implants, and the recovery bays) → **hard pre-op gates, no wheel-in while any is open:** anesthesia review with ASA grade · procedure + anesthesia consents (guardian path per D-31 for minors; POCSO intimation where triggered) · site marking · NPO status · payment/deposit clearance · **escort verified — a day-care patient discharges to a responsible adult, structurally** · the statutory gates below → **WHO checklist states Sign-in → Time-out → Sign-out** (§11.9; count-mismatch hard stop inherited) → intra-op record with §11.9's timestamps; **implant scan-on-use is a consignment event** — charge + patient sticker + vendor liability in one event (§11.19-A cath-lab pattern), min(tariff, MRP, ceiling) applies (§11.19-C-3), the §31(7) six-month deemed-supply clock runs per lot (§11.19-D-7), 3-way match per case (§11.16) → **recovery bay with scoring-to-threshold discharge readiness** (§11.9's PACU pattern; thresholds are definition data) → **same-day discharge: the lighter cascade, gates intact** (map 11) — wound-care instructions + follow-up booking through the gateway; **missed follow-up = clinical recall task, not a no-show.**

**Statutory machinery — all pre-existing law, first live instantiation here:** **MTP:** the unit performs MTPs — `termination.recorded` register (§11.17) with gestation-limit opinion counts as MTP-Act configuration, statutory consent forms, sealed event class (§11.19-C-5); **the facility's MTP approved-place certificate is a §19 gate for this procedure class.** **PCPNDT:** gynae runs USG — the Form-F structural gate (§11.7, widened by §11.19-C-6 to any USG on a woman of reproductive age) applies in the unit exactly as in radiology: an applicable scan cannot close without `form_f.recorded`; machine and sonologist registrations are governed config; **PCPNDT registration is a §19 gate.** **MLC:** ortho trauma runs the MLC check before procedure (§11.14).

**CSSD-lite:** the adjacent autoclave runs §11.10's set-based cycle unchanged — barcode-tracked sets → sterilization batch → **biological indicator per load; BI fail = every set in that load auto-recalled** → sterile issue against tomorrow's list. The mini-OT is the **first live consumer of the `cssd.*` event family** defined at v3. Autoclave down or no valid set = case postponed, reason-coded (§11.9's exception grammar). The full CSSD department still arrives with the major suite.

**Overnight conversion — the honest two-system rule:** a patient not dischargeable same-day converts via `daycare.converted_to_admission` — and **until the IPD cluster ships, the admission itself is a documented handoff to the incumbent system under E-11's transition-operations boundary map**; the event records the crossing so the journey stays auditable across the boundary. A complication beyond the unit's scope → stabilize + transfer (ambulance, referral note), evented.

**Exceptions (adds to §11.9's set):** escort absent at discharge = discharge blocked, evented · Form-F incomplete = no scan and no wheel-in · BI fail mid-list = affected sets quarantined, re-kit or postpone · consignment implant absent = postpone decision, evented · same-day return to theatre (bleeding) = unplanned-return flag (§11.16's quality counter) · conversion with no incumbent-system bed available = ER/transfer path.

**Resources (Plan 13 registry contributions):** theatre ×1 · recovery-bay beds ×2 (day-care class) · autoclave (device, cycle logs) · instrument sets · consignment store location.

**New events:** daycare.booked · daycare.checked_in · escort.verified · daycare.discharge_ready · daycare.discharged · daycare.converted_to_admission · form_f.recorded (shared with §11.7's imaging gate) — **catalog ~252** (reuses consignment.deployed, cssd.load_sterilized / cssd.bi_failed / cssd.set_recalled, termination.recorded).

**Staffing (S10 alignment):** OT sister + circulating nurse + roster-resolved anesthetist + CSSD duty; the on-call ladder resolves through the Plan-03 roster seam.
...
- **(v4.8) PRE-MINI-OT GATES (§11.16-A):** MTP approved-place certificate on file before the MTP procedure class activates · PCPNDT registration (machine + sonologists) on file before any USG in the unit · day-care case-selection criteria approved as Class-B definition data · ortho-implant consignment agreements reviewed by counsel (rides the partner-agreement review already in flight).
