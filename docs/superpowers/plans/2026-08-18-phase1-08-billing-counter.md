...
- **DB kernel:** `Db`, `Tx`, `withTx(db, fn)`, `createDb(url)` (`kernel/db/client.ts`) · schema barrel `kernel/db/schema/index.ts` (9 re-exports; T1 adds `./billing`) · `truncateAll` (`test/helpers/db.ts` — six statements; T1 adds ONE self-contained billing statement; billing tables FK only into `patients` (real) and use text refs elsewhere, so they need their own statement naming all fourteen tables, placed BEFORE the patients statement or self-contained with no FK into a later-truncated table — see T1) · migration journal at idx 10 (`0010_silent_victor_mancha`), 11 applied; next generated tag is `0011_*`, then the custom `0012_billing_immutability` · precedents: `bigserial("seq", { mode: "number" })` non-PK (`tariff.ts:83`), partial `uniqueIndex().where(sql...)`, jsonb columns, `date(…, { mode: "string" })` round-trips `'YYYY-MM-DD'` (Plan 07 flag ① discharged).
...
- **Additive-only over shipped code.** Files modified outside `modules/billing/`, exhaustively: `kernel/db/schema/index.ts` (one re-export, T1) · `test/helpers/db.ts` (ONE added statement, T1) · `apps/core/package.json` (two script lines, T3) · `src/modules/opd/consultation.ts` + `src/modules/opd/errors.ts` + `src/modules/opd/index.ts` + `src/modules/opd/consultation.test.ts` (T10, the guard hook — exhaustive) · `src/app.module.ts` (T11) · root `README.md` (T12, T16). Every kernel folder byte-frozen; `modules/tariff/**` byte-frozen; `modules/patients/**` byte-frozen; `packages/contracts/**` byte-frozen; `.github/workflows/**` untouched (tripwire 10).
...
apps/core/
  ...
  test/helpers/db.ts                                 T1  ONE added statement (fourteen billing tables, self-contained)
...
- Modify: `src/kernel/db/schema/index.ts` (+ `export * from "./billing"`), `test/helpers/db.ts` (ONE statement)
...
- [ ] **Step 3: Fail-first (§3.23 subset deploy).** `time.test.ts` (4): Mar 31 23:59 IST → fy "2025-26"; Apr 1 00:00 IST → "2026-27" (both from UTC instants — hand-derived: `2026-03-31T18:29:59Z` is IST Mar 31 23:59:59; `2026-03-31T18:30:00Z` is Apr 1); fyShort "26-27"; istDay of `2026-08-31T18:30:00Z` = `"2026-09-01"`. `billing.test.ts` (8): the fourteen tables insert/select round-trips (spot: invoices money columns come back as numbers; jsonb candidates round-trips), the two partial/unique arbiters (one-live-session index: second open 23505; invoice_lines (invoiceId, lineNo) 23505), seq bigserials populate ascending, K1's truncate red (run against the unextended `db.ts` FIRST — every test dies at `beforeEach` if the statement is missing; then extend and go green). `immutability.test.ts` (8): for each of the six protected tables, an `UPDATE` raises with message containing `billing_immutable`; two `DELETE` samples (invoices, allocations) raise likewise — all eight EXECUTED, red before 0012 exists, green after.
...
- [ ] **Step 5: Truncate.** ONE added statement in `truncateAll`, BEFORE the patients statement: `truncate table daily_closes, recon_batches, entered_in_error_marks, refund_vouchers, allocations, receipt_tenders, receipts, credit_note_lines, credit_notes, invoice_lines, invoices, cashier_sessions, document_series, billing_config` (self-contained: the only outbound FKs go to `patients`, truncated LATER in its own statement — Postgres only requires FK-parents truncated in the same statement as their children when the PARENT is being truncated; `invoices → patients` needs `invoices` gone before `patients`, which this ordering guarantees).
...
- [ ] **Step 7: Run to pass.** Core **95 suites / 552 tests** (from 93/536: +time 4, +schema 8, +immutability 8 — one schema test is the K1 red converted). Verify green.
...
- [ ] **Step 6: Run to pass.** Core **99 suites / 585 tests** (+10 +9 +12 +2). Verify.
...
**Acceptance criteria:** 33/33 + red-first; every fixture value matches this document's derivations (gate re-derives B-01, B-03, B-06 by hand); manifest asserts the ten names; mutants M-B01/02/03/05 all DIED 3/3; purity + float greps green; core 99/585; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **103 suites / 604 tests** (+7+8+2+2). Verify.
...
**Acceptance criteria:** 19/19 + red-first; race measured 10× isolated with output-confirmed isolation; validate gate catches all four breaks THROUGH the runtime loaders; seed idempotency quoted twice; both package.json lines; core 103/604; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **105 suites / 619 tests** (+11+4). Verify.
- [ ] **Step 5: Commit** — `feat(core): cashier sessions — one-live arbiter, denomination close, SoD-guarded variance approval` → pull → push.

**Acceptance criteria:** 15/15 + red-first; the SoD refusal exercised through the kernel (not a local check); open race measured; mutants DIED 3/3; shaping disclosed; core 105/619; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **107 suites / 640 tests** (+15+6). Verify.
...
**Acceptance criteria:** 21/21 + red-first; atomicity of test 3 asserted from row counts; all four mutants DIED 3/3; every money expectation hand-derived in comments; core 107/640; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **109 suites / 661 tests** (+12+9). Verify. **Pipeline A ends here.**
...
**Acceptance criteria:** 21/21 + red-first; both races measured 10× with output-confirmed isolation; the lock-observation leg holds the invoice row from a raw client; M-A1 observation discriminates; M-A2/A3 DIED 3/3; core 109/661; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **110 suites / 673 tests** (+12). Verify.
...
**Acceptance criteria:** 12/12 + red-first; B-06's numbers land verbatim as rows (gate re-derives); M-C1/2/3 DIED 3/3; the ASKED amount in `over_cap` detail; core 110/673; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **111 suites / 685 tests** (+12). Verify.
...
**Acceptance criteria:** 12/12 + red-first; guard-1 test built on a genuinely partially-paid (dues) invoice; advance race measured; M-R1/2/3 DIED 3/3; core 111/685; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **112 suites / 694 tests** (+9). Verify.
...
**Acceptance criteria:** 9/9 + red-first; expected-net hand-derivation in comments; M-N1/N2 DIED 3/3; core 112/694; verify green; clean.
...
- [ ] **Step 5: Run to pass.** Core **115 suites / 713 tests** (+8 +4 +4, +3 in the OPD suite). Verify.
...
**Acceptance criteria:** 19/19 + the OPD suite's red quoted on the missing registry import; OPD edits are EXACTLY the four named files; guard test 1 proves shipped behaviour unchanged; M-G1/2/3 DIED 3/3; gstr1 sums stored heads (mutant-proven); core 115/713; verify green; clean.
...
- [ ] **Step 4: Run to pass.** Core **116 suites / 724 tests** (+11). Verify.
...
**Acceptance criteria:** 11/11 with the red-first 404 run quoted; route table implemented verbatim; 403 sweep green over every route; the OPD 409→pay→start leg green over REAL HTTP; app.module edits are the two named lines; core 116/724; verify green; clean.
...
- [ ] **Step 3: Run to pass.** Core **117 suites / 733 tests** (+9). Root verify (whole repo). **Pipeline B ends here.**
...
**Acceptance criteria:** 9/9 (no red owed — stated); every total in stories 8–9 hand-summed in comments from the story's own numbers; README section present with the statutory-anchor table; core 117/733; verify green; clean.
...
|---|---|---|---|---|---|
| K1 | T1 | truncate statement clears the billing chain | = pre-fix red: unextended `db.ts` → FK truncate error in every `beforeEach` | | §3.12 executed |
...
| K4 | T1 | FY flips at IST Apr 1, not UTC | M-T1 (fy from UTC date) → Mar 31 23:59 IST lands in the next FY | | pure boundary |
...
8. **The billing truncate statement must precede the patients statement** (invoices FK patients); placed before it and self-contained — verified against the shipped six-statement order.
...
`apps/core`: 93 suites / 536 tests → **T1** 95/552 → **T2** 99/585 → **T3** 103/604 → **T4** 105/619 → **T5** 107/640 → **T6** 109/661 → **T7** 110/673 → **T8** 111/685 → **T9** 112/694 → **T10** 115/713 → **T11** 116/724 → **T12** 117/733. `apps/web`: 21 files / 80 tests → **T13** 27/103 → **T14** 28/110 → **T15** 29/119 → **T16** 30/127. `packages/contracts` 3/7 unchanged throughout. Per-suite deltas for the two touched OPD web suites (`opd-consult` +3, `opd-vitals` ±0) and the OPD core suite (`consultation.test` +3) are stated as deltas — the pre-compile scout measures absolutes.
