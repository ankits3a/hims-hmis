...
**STATUS: SHIPPED 2026-08-15 — 6/6 tasks gate-passed, `03969ec` → `5fd3ad0`, CI green on all six. `apps/core` 68 suites/383 tests → 69/396; exactly one migration (`0009_huge_joshua_kane`). Gate report: [`reports/plan-06-2-gate-report.md`](reports/plan-06-2-gate-report.md).** The A1 ruling was Option B (database-side `seq`), owner in-conversation 2026-08-15, and T1 shipped as written.

> **⚠ ONE PREDICTION IN THIS DOCUMENT WAS FALSIFIED BY EXECUTION.** Assertion Book row **K5** and the Task 2 Step 1 Test 1 code block are WRONG as originally written — the test they specify cannot discriminate a working activation serializer from none, which is the very defect (audit A2) this plan existed to fix. Mutant A SURVIVED 5/5 against it. Both are corrected in place below, with the falsified text kept for the record; the shipped suite holds a *different* row and Mutant A DIES 5/5. See the correction banner in Task 2 Step 1, gate report §7, and ledger §3.28. **This is the plan's own rule working on the plan itself: an unexecuted discrimination claim is a prediction, and measurement beats this document.**
...
| `services.test.ts` scaffolding (stress-verified) | `test(…)` dialect; module-scope `const actor: Actor = { type: "user", id: "u1" }`; `beforeEach(truncateAll)`; history test binds `r1`/`r2`/`r3` exactly as quoted in T1 Step 5; the file imports NOTHING from `drizzle-orm` or the schema (T1 adds both) | full transcription at `acd42d2` |
| `seq`-widening blast check (stress-verified) | **No shipped `toEqual` deep-equals a full `regulated_prices` or `adjustment_rules` row** — every raw-row consumer (`rules.test`, both e2e files, controller passthroughs) asserts via `.toHaveLength`, field access, or `.map(r => r.id)`; every full-shape `toEqual` compares a curated projection (`resolveRegulatedPrices` map, `loadRuleConfig`'s `AdjustmentRuleConfig`/`ManualCaps`). `apps/web` has zero tariff references | toEqual sweep over all tariff test files at `acd42d2` |
...
- **Scratch mutants:** built beside the source as `*.mutant.ts` (implementation copies) + `*.mutant.test.ts` (specs), run isolated by explicit path (`pnpm --filter @hmis/core exec jest --passWithNoTests <path>`), verdicts recorded, then **deleted BEFORE any workspace-count verification and BEFORE commit** (tripwire 4; jest's testMatch would otherwise collect the spec). `git status` clean before every commit. **Mutant specs are SELF-CONTAINED: never import from a shipped `*.test.ts`** — importing a test file registers its tests inside the importing suite — copy the needed seeding/fixture blocks inline instead (helpers under `test/helpers/` are importable; test FILES are not).
...
```
  ...
  Note the final line (`history[0]!.seq …`) does not COMPILE against shipped code — `RegulatedPriceRow` has no `seq` until Step 3 — so **Step 2 runs the test with exactly that one line commented out**, restoring it in Step 5 (§3.23: the fail-first variant must compile and run against unmodified shipped code on its own; the defect it demonstrates is the resolver's, not the type's). The `ceilingPaise` history assertion stays in: pre-fix it is itself part of the red evidence (same-date head is id-random).
...
- [ ] **Step 5: Restore the two seq assertions** in the Step-1 test; upgrade the shipped history test in place — `"listRegulatedPrices: one service's full history, newest first, same-date correction before its original"` gains heap agitation between the r3 insert and the query. Add BOTH imports — scout-verified absent: `services.test.ts` imports nothing from `drizzle-orm` and nothing from the schema today — `import { eq } from "drizzle-orm";` and `import { regulatedPrices } from "../../kernel/db/schema";`:
...
> ## ⚠ POST-EXECUTION CORRECTION (2026-08-15) — Test 1 below, AS WRITTEN, DOES NOT DISCRIMINATE
>
> **This block shipped in corrected form; the version printed below is the falsified one, kept for the record.** `SET_LOCK_SQL`'s predicate (`status in ('submitted','activated')`) covers **the target version's own row** — it is `submitted` at that moment — and `activateVersion`'s single-winner conditional UPDATE (`where id = versionId and status = 'submitted'`, `versions.ts:220-224`) must take an exclusive lock on that same row **whether or not the serializer exists**. So a serializer-less `activateVersion` blocks too, `expect(state).toBe("pending")` is satisfied by both implementations, and the test cannot tell them apart. **Measured: Mutant A SURVIVED 5/5**, independently reproduced by the gate at SURVIVED 3/3.
>
> **The shipped fix:** the holder locks a **different** row — a second submitted-and-approved version, seeded *after* the target (so the target keeps `versionNo` 1) — via `select id from tariff_versions where id = $1 for update`. That row is inside the serializer's set predicate and outside everything the target's own activation reads or writes, so the shipped code blocks on the set lock and the mutant settles immediately. **Mutant A then DIED 5/5.** Both invariant assertions stay unconditional at exactly 1: the second version is only ever *submitted*, never activated. `SET_LOCK_SQL` remains a module const because Test 2 still uses it verbatim.
>
> This is ledger **§3.28**: a test that observes a lock must hold a row OUTSIDE the target's own write path, or the mechanism and the row-level lock that would block anyway are indistinguishable. It is §3.21/§3.25's family, third variant — and note that this plan's two authoring passes, its stress pass and the compile review all read K5 as sound. Only building the wrong implementation and watching it pass caught it. See [`reports/plan-06-2-gate-report.md`](reports/plan-06-2-gate-report.md) §7.
...
|---|---|---|---|---|---|
> **EXECUTED 2026-08-15 — the verdict column below is now MEASUREMENT, not prediction.** Nineteen rows, nineteen executed verdicts; eleven mutants were independently rebuilt and re-run by gates rather than accepted from a coder report. Read "19/19 executed" carefully: it is **not** "19/19 killed". K5's prediction was falsified and its mutant died only after the test was corrected (banner in Task 2 Step 1), and K2 SURVIVED exactly as this document authorised. Full evidence: [`reports/plan-06-2-gate-report.md`](reports/plan-06-2-gate-report.md) §6.

| K1 | T1 | 60 same-date corrigenda ALL resolve to the correction | shipped id-tie-break → some service resolves `ceilingPaise 99900` (≈88%/run window) | **RED 15/20 isolated (75%)**, exact predicted shape; gate independently rebuilt the mutant → **DIED 5/5** | observed rate 75% vs the derived ~88% — §3.22 again: a derived race rate is a possibility, not a forecast. Deterministic green post-fix (5/5) |
| K2 | T1 | history `[r3, r2, r1]` with heap agitation | drop `desc(seq)` → order decided by Postgres sort internals | **SURVIVED 10/10** — measured, not predicted; gate independently reproduced **SURVIVED 10/10** | the authorised honest outcome: the audit proved this row un-predictable (m11) and it is. Load-bearing defense is structural (bigserial allocation) + K1's executed red. The coder reported the survival rather than engineering a kill — which is the point |
| K3 | T1 | rules cap last-write-wins rides `seq` | `asc(seq)`→`asc(id)` revert — **no behavioural kill exists**: 0/200 same-ms on the sequential upsert path | **none built — declaration honoured** | the one un-killable row, stated up front and not manufactured; defenses: bigserial allocation order + A17's `duplicate_manual_cap` |
| K4 | T1 | corrigendum's `seq` > original's, in-tx | (structural pin of allocation order; green by construction) | **green, in-tx allocation order confirmed** | belt for K1's mechanism claim |
| K5 | T2 | activation BLOCKS ≥400ms under a held set lock, completes after release | Mutant A (serializer deleted) → settles immediately → "pending" assertion fails | ⚠ **PREDICTION FALSIFIED, then DIED 5/5.** As written the test held the TARGET's own row, which the single-winner UPDATE locks anyway → **SURVIVED 5/5** (gate reproduced 3/3). Corrected to hold a SECOND version's row → **DIED 5/5** (gate reproduced 5/5) | "deterministic by design" was wrong — see the correction banner in Task 2 Step 1, gate report §7, ledger §3.28 |
| K6 | T2 | forced same-version loser typed `not_submitted`; 1 row, 1 event | Mutant B (`ne()` deleted) → loser `effective_from_not_monotone` | **DIED 5/5**; gate independently rebuilt → **DIED 5/5** | symmetric in FIFO order — not a §3.13 flake. The forced interleave does reach the in-tx re-check, as designed |
| K7 | T3 | mis-bound REJECTED approval → `approval_subject_mismatch`, vA stays submitted | shipped order → `approval_rejected` + vA terminally rejected | **RED**, observed `{code: approval_rejected, status: rejected}`; gate re-derived against a pre-fix module copy → **RED** | the audit's B1 reproduction as a test |
| K8 | T3 | drafter on rejected approval → `sod_drafter_activator`, version stays submitted | shipped order → belt fires for the SoD-excluded actor | **RED**, same observed pair; gate re-derived → **RED** | shipped `:122` test keeps pinning the legitimate belt path, and passed byte-unchanged post-reorder |
| K9 | T3 | wrong-subjectTYPE approval refused with matching subjectId | Mutant C (subjectType half deleted) → vA ACTIVATES | **DIED 5/5**; gate independently rebuilt → **DIED 5/5** | green-first (m7 is a coverage gap). Flag ② held: `requestApproval` writes `subject` verbatim, no normalisation |
| K10 | T4 | cap 10000, ask 60000 > gross 50000 → `over_cap` @60000, detail `"…10000bps…"` | Mutant D (clamped operand) → accepted `{50000, requiresApproval: true}` | **DIED 5/5** (observed accepted `{50000, requiresApproval: true}`, exactly as hand-derived); gate independently rebuilt → **DIED 5/5** | the ONE cap value where operands disagree — B4's exact revert |
| K11 | T4 | gross 0, ask 5000 → `over_cap` @5000, detail `"…of 0p"` | Mutant D (same build) → accepted `{0, requiresApproval: false}` | **DIED 5/5** (observed accepted `{0, rejected: null}`, exactly as hand-derived); gate independently rebuilt → **DIED 5/5** | m1 policy pin; one mutant build, two kills, both legs measured separately |
| K12 | T4 | undefined/undefined bounds → `regulated_price_missing` | shipped `=== null` → net 5600, clamp null | **RED**, no throw observed on the shipped guard; gate independently built the mutant → **DIED** | cast-built ctx; 0-paise bound survives `==` |
| K13 | T5 | non-Zod load failure → `rule_config_load_failed`, zero `manual_caps_missing` | shipped bare catch → four `manual_caps_missing`, no typed code | **RED**, observed four `manual_caps_missing` and no typed code, exactly as predicted | ZodError path pinned unchanged by the shipped Break-4 test, which passed byte-unchanged. Flags ③ and ④ both held on the first attempt |
| K14–16 | T5 | retired / expired / future second cap → NOT duplicate | E1/E2/E3 (each filter deleted in scratch copy) → phantom duplicate reported | **DIED 1/1 each** (retired / FY25 / FY27 legs); gate independently rebuilt all three → **DIED 3/3** with the unmutated baseline passing | one test, three legs, three sequential scratch edits. No production change was owed for m6 — the filters were already correct |
| K17 | T6 | e2e history for A excludes B's row; B returns its own | Mutant F (route ignores `:id`) → 3 items where 2 asserted | **DIED 5/5** (observed 3 items where 2 asserted) | executed at controller-unit level — approximation declared in the report as required; route binding, auth and serialization stay covered by the shipped e2e's captured-id assertions |
| K18 | T6 | raw `readdirSync` listing equals the manifest | G1 (`g99-rogue.JSON` straggler in scratch dir) → listing ≠ files | **DIED** (`g99-rogue.JSON` present in `readdirSync`, absent from `files`) | scratch-directory replication, declared. Shipped fixtures byte-untouched |
| K19 | T6 | no fixture has `lines: []` (price: expected length = lines length) | G2 (gutted g01 copy in scratch dir) → length 0 | **DIED** — but via `fixtureSchema.parse()` throwing `ZodError` (the schema already enforces `lines.min(1)`) before the length assertion is reached | same scratch run. The earlier death was observed and reported, not treated as a defect: the assertion has teeth either way and the schema is a welcome second line of defence |
...
**Pass 3 (post-commit stress test, 2026-08-15, before compilation — four defect classes fixed by five edits in the follow-up commit, and the two risks Pass 2 had left open closed by a dedicated scout):**
13. **T2's `let results` after try-finally would not typecheck** — TypeScript's definite-assignment analysis does not credit a try-block assignment for use after the block. Initialized to `[]`, which also makes an aborted try fail loudly at the length assertion instead of throwing a confusing TS-level error at authoring time.
14. **Mutant specs could have imported shipped test files for their seeding** — importing a `*.test.ts` registers that suite's tests inside the importing spec, corrupting counts and evidence. Global Constraints now mandate self-contained mutant specs (inline seeding copies; `test/helpers/` remains importable), and T5's Step 4 wording was made explicit.
15. **T1 Step 2's comment-out instruction said "two" assertions where exactly ONE line fails to compile** (`history[0]!.seq …`) — imprecision a literal coder would trip on; and the `ceilingPaise` history assertion is itself pre-fix red evidence, so commenting it would have discarded signal. Made exact.
16. **T1 Step 5's imports were conditional** ("only if not already present") — a §3.3 conditional instruction. The stress scout transcribed `services.test.ts` in full: it imports nothing from `drizzle-orm` and nothing from the schema, so the instruction is now unconditional.
17. **The `seq`-widening risk was closed by sweep, not assumption:** every `toEqual` in every tariff test file was classified (scout, source-verified) — none deep-equals a full row of either migrating table, so no shipped assertion breaks when the rows gain `seq`. (The same sweep flagged `gst-config.test.ts:34` as the one full-row-`toEqual`-shaped assertion in the module — different table, out of scope, noted for whoever next migrates `gst_config`.)
