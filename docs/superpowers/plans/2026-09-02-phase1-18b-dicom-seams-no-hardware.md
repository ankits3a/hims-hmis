...
- **F9 (T1, CI)** — `test/ist-clock-parity.test.ts` pins the census of hospital-clock copies across the workspace; `mwl.ts`'s offset was the thirteenth and the task's own suites could not see it (the file runs only in the workspace run — the lab recorded the same sentence for the twelfth). Declared in the census with its argument, derived from the kernel's export. **§2.138's lesson a third time: a workspace census is found by the workspace run, never by the task's batch.**
...
**Verdicts over the 25 fixes: 31 CORRECT · 4 INCOMPLETE · 1 WRONG · 6 NEW (1 MAJOR, 5 MINOR). All closed in the second remediation commit.**
- **INCOMPLETE A2** — the dump format (the one the bridge pulls) carried no `malformedAeTitle`; the AE regex admitted `]` and all-spaces. Now: a header line on the dump + `X-Hmis-Mwl-Malformed-Ae-Title`; the regex excludes both, pinned.
- **INCOMPLETE B1** — the human tier was right, but the technique line carried the book's LABEL ("USG pelvis (female)") into the lockout for exactly the women the type exists for. Now the drafter takes the patient's tier as a fact and names the modality alone when the label would hit; the refusal path stays for a drafter that puts such a word in a finding. Pinned both ways (a 30-year-old woman → "Ultrasound.", a 62-year-old man → the full name).
- **INCOMPLETE B2 / NEW-2** — the server refused signing the proposal, but the screen still picked it as the signable and as the reload seed. Now a machine draft is never the signable nor the seed; Sign is disabled until a human draft exists, pinned.
- **INCOMPLETE C4** — the second read was back one hook over (the seed query fetched the proposal, with its PHI row). Closed by the same rule: the seed is a human draft only; the test asserts the proposal is never fetched.
- **WRONG C7 (NEW-1, MAJOR)** — the bridge script swapped the worklist DIRECTORY; under Orthanc's bind mount that unlinks the mounted inode's files for ever. Now each `.wl` lands by an atomic rename INSIDE the directory and stale names are removed; `flock` (NEW-4), `login()` returns curl's status (NEW-3), `umask 077` for the token.
- **NEW N1/N2 (MINOR)** — `studyImageViews` is a LEFT join (a view row can never vanish); `read.ts` imports the door's own permission string.
**The method's measurement:** pass 1's 25 fixes were 31 CORRECT / 4 INCOMPLETE / 1 WRONG on inspection — about one fix in six left the dimension open one path over, the same shape 18a measured at 15 of 16 and RC-4 at 1 WRONG. Pass 2 paid for itself on C7 alone.

### 8.6 Actuals vs stop-loss (1,720,000)
Rough, from the session's own counter and the reviewers' reported usage: main session ≈ 380k, pass 1 ≈ 400k (three reviewers), pass 2 ≈ 270k (two) — **≈ 1.05M, 61% of the stop-loss**, five tasks and two review passes. The review term (≈ 670k) is again the term that paid: 3,516 → 3,516+ passing tests and every named mutant dead found none of the two CRITICALs; reading did.
