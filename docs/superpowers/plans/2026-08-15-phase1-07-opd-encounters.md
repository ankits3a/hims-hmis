...
### D8. Events minted (17 names, all `module: "opd"`; the three ratified additions are marked ★)
...
apps/core/
  ...
  src/modules/opd/events.ts                         T2  the 17 catalog definitions + qr.signature_failed (opd)
...
- **Events note:** seventeen catalog names + `qr.signature_failed`, all `module: "opd"`; three ratified catalog additions (`queue.called`, `queue.skipped`, `visit.abandoned`) to be recorded in the gate report; the dispatcher stays unscheduled until Plan 11 and no OPD consumer subscribes through it — realtime rides the gateway's per-process tail.
