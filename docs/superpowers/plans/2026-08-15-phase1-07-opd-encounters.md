...
apps/core/
  ...
  (repo ROOT) README.md                             T10 (OPD module + runbook) · T16 (web) — NOT apps/core/README.md, see erratum E1
...
GET    /opd/me/doctor                     opd.consult           → DoctorRow | 404 { code: "not_a_doctor" }   [E3]
...
> **ERRATA E3–E5 (2026-08-18, recorded after T9 shipped — these describe what is SHIPPED and gate-verified; pipeline C consumes this surface, so read them before writing any screen).**
>
> **E3 — `not_a_doctor` is 404 on `GET /opd/me/doctor` and 400 everywhere else, and that is deliberate.** Two of this plan's own tables contradicted each other: the route table above specifies a **404** for that route, while the error-mapping table puts `not_a_doctor` in the **400** set. T9 implemented both literally, which is the only faithful reading — the route throws its documented 404 directly, and the generic `toHttp` mapping is untouched, so a *service-raised* `not_a_doctor` (e.g. from `requireTreatingDoctor` on a consult route) still maps to 400. **Screens must therefore treat 404 from `/opd/me/doctor` as "this user is not a doctor" and not as a transport error.**
>
> **E4 — verify-by-execution flag ⑫ had no home in this task's prescribed tests and was discharged by extending test 6.** None of the six e2e tests the plan specifies posts a `slotStart`, so the plan's own e2e could not discharge its own flag. T9 extended test 6 with a `GET /opd/slots` → `POST /opd/appointments` leg rather than adding a seventh test (which would have broken the 6/6 count) or declaring the evidence unobtainable. The suite is still exactly 6 tests and two more contract routes gained coverage. Flag ⑫ is **discharged**.
>
> **E5 — the status code of every POST not annotated above is Nest's default 201, and the e2e pins it.** The table annotates 201 on eight POSTs and `@HttpCode(200)` on exactly one (`/opd/prescriptions/verify`); it is SILENT on the rest, and silence means **201**. Concretely these all return **201**: `/opd/appointments/:id/check-in`, `/opd/appointments/:id/reschedule`, `/opd/appointments/:id/cancel`, `/opd/visits/:id/abandon`, `/opd/visits/:id/re-enter`, `/opd/visits/:id/consult/start`, `/opd/visits/:id/consult/complete`, `/opd/queues/transfer`, `/opd/queues/:sessionId/call-next`, `/opd/queues/:sessionId/status`, `/opd/queues/entries/:entryId/skip`, `/opd/leaves/:id/cancel`. **Pipeline C screens must not assume 200 on any of them**, and any test that stubs these routes must stub 201. (Note this contradicts the plan's own leg-5 prose in T10, which said reschedule and leave-cancel return 200; the tests assert the honest 201. If the owner ever wants 200 there it is ten `@HttpCode(200)` decorators plus matching e2e expectations — decide before screens ship, not after.)
...
- Modify: **`README.md` at the repo ROOT** (an "OPD module (Plan 07)" section + "Go-live runbook — OPD" + "Realtime (WebSocket) protocol") — *erratum E1: this said `apps/core/README.md`, which does not exist; the repo has exactly one README.md, at the root, and it is where the Plan 05 and Plan 06 module docs already live.*
...
① `date(…, { mode: "string" })` round-trips `'YYYY-MM-DD'` with no TZ shift — T1 schema test 1. ② drizzle-kit emits the partial unique index WITH the `IN ('booked','checked_in','needs_rebooking')` predicate — T1 Step 3 inspection + schema test 2 (STOP if dropped; never hand-edit). ③ ~~drizzle `Tx` is not assignable to `Db`~~ — **FALSIFIED by execution (T3): `Tx` IS assignable to `Db`.** T3 probed it and then added a deliberate type error to prove the probe file was in `tsc`'s scope before trusting the negative. The plan pre-authorised either outcome, so the Db-first/`…InTx` split shipped unchanged and is correct — but do NOT repeat the claim as fact. ④ `fast-check` ^3 under ts-jest CJS — T5's property suite. ⑤ `seed:opd` idempotent — T6 runs it twice, both outputs quoted. ⑥ `ws` under pnpm 10's denied build scripts (optional native peers absent) — T8's gateway suite connects a real socket. ⑦ `name = any($1::text[])` binding via `sql.param` — T8 tail tests 2/5 (the `dispatcher.ts:26` shape). ⑧ the gateway suite exits without open handles (`unref()` + `stop()`) — T8 acceptance 4. ⑨ the perf seed (18k+18k+200k rows) completes inside the 120 s `beforeAll` in CI — T10 CI green. ⑩ `HttpAdapterHost.httpAdapter.getHttpServer()` is available in `onApplicationBootstrap` and an `upgrade` listener attached before `listen` fires for later connections — T8 gateway tests. ⑪ drizzle `.for("update")` on a select compiles — **and the "serializes" half is NOT discharged, see erratum E6.** ⑫ `z.coerce.date()` accepts ISO `slotStart` bodies — **discharged by T9's e2e test 6 as extended, see erratum E4** (none of the plan's six prescribed tests posted a `slotStart`). ⑬ `defineEvent(...).make` accepts `encounterId` + `correlationId` and `appendEvent` writes both columns — T3 test 1 asserts the columns. ⑭ Nest `HttpException` with an object body returns `{ statusCode, message, code, detail }` verbatim — T9 e2e test 3/4. ⑮ jsdom lacks `speechSynthesis`/`SpeechSynthesisUtterance` (assign via `vi.stubGlobal`) and its `WebSocket` is replaced by the fake — T11/T16; the list of jsdom gaps may be incomplete. ⑯ TanStack Router `validateSearch` for `/opd/display?rooms=` — T16. ⑰ `Test.createTestingModule` + `app.listen(0)` + a `ws` client in jest — T8/T10. ⑱ `<input type="time">` values under `user-event` in jsdom — T11 schedules test. ⑲ `Object.keys` set equality as the "no identity on the board" proof — T5 test 9 / T9 test 5 / T16 test 4. ⑳ `sql\`select coalesce(max(seq), 0)::bigint\`` returns a numeric string through node-postgres — T8 tail (`Number()` applied).

> **ERRATUM E6 (2026-08-18) — flag ⑪ is falsified in its second half, and the honest statement is that the UNIQUE INDEX carries correctness, not the lock.** The flag bundled two claims, "compiles" and "serializes". The first is discharged by the code building. The second is discharged by nothing: T7's gate rebuilt Mutant P1 (the encounter-row `FOR UPDATE` dropped) and ran the version race isolated **10 times — SURVIVED 10/10**, independently reproducing the coder's 0/20. Prescriptions test 4 pins the *outcome* (versions {1,2}, one active row, two events), which the unique index on `(encounter_id, version)` already guarantees, so the test cannot tell the two implementations apart. The only evidence for "serializes" was a throwaway two-transaction probe that was deleted and is now unreproducible. **The `FOR UPDATE` is retained (it is not wrong and it avoids relying on a constraint violation as flow control), but no plan may cite flag ⑪ as proof that it serializes.** Two authoring rules follow: a flag that bundles two claims must name a separate discharging assertion for each, and a flag whose only evidence is throwaway scratch is not discharged — commit the probe or mark the flag declared-unproven with its structural defence named.
...
- **ERRATA INDEX (all landed 2026-08-18, after pipelines A and B shipped — read before compiling pipeline C):** **D8** the catalog is EIGHTEEN P1 names + `qr.signature_failed` = 19 `defineEvent` calls, not seventeen (one stale "seventeen" is left deliberately inside T2's code block because it matches the shipped source comment, and `events.ts` is frozen) · **E1** the docs file is the repo-ROOT `README.md`; `apps/core/README.md` does not exist and never did · **E2** the `boardSnapshot` budget is **500 ms**, not 300 — the 300 ms ceiling was authored from an isolated measurement and fails 1 run in 3 under parallel load · **E3** `not_a_doctor` is 404 on `GET /opd/me/doctor` and 400 when service-raised elsewhere; two of this plan's tables disagreed and both readings shipped deliberately · **E4** flag ⑫ had no home in T9's six prescribed tests and was discharged by extending test 6 · **E5** every POST not annotated in the route table returns Nest's default **201** (twelve of them), and the e2e pins that — screens must not assume 200 · **E6** flag ⑪'s "serializes" half is FALSIFIED (Mutant P1 SURVIVED 10/10); the unique index carries correctness · **flag ③** FALSIFIED — drizzle's `Tx` IS assignable to `Db`.
- **Pipeline C prerequisites (measured, 2026-08-18):** baseline is `apps/core` **93 suites / 536 tests**, `packages/contracts` 3/7, `apps/web` **11 files / 37 tests** — the plan's web ladder (13/48 → 16/57 → 17/63 → 18/68 → 20/76 → 21/80) still starts from 11/37 and is unaffected by A/B, but `apps/core`'s 535 in the T10 row became **536** when remediation R1 added one tail test. **C installs nothing and must not touch `apps/core/**` at all** — every C task is `apps/web` plus the shared router/locales/styles. Re-measure per-suite counts with ONE scout before compiling (§2.9); the measurement beats every number here.
- **Open defects carried into pipeline C, none blocking:** `registerPatient` derives `dob` from the wall clock instead of taking `now: Date = new Date()`, contradicting this plan's own Global Constraint — the durable fix for the §3.31 date bomb, and `test/helpers/opd.ts`'s `mkPatient` still defaults to `ageYears: 30`, so the next hard-age assertion anywhere re-arms it (needs a plan owning `modules/patients/`) · the lifecycle e2e's `FrameStream.waiter` is a single slot, so any future *parallel* `await` on two frames would silently clear one (dormant; every shipped await is sequential) · `opd-lifecycle.e2e.test.ts` needs a bookable slot 20+ minutes ahead on today's template, so CI goes red for calendar reasons roughly 23:30–midnight IST (fails loudly by design) · the gateway checks `hasPermission` only at SUBSCRIBE, never on fan-out, so a revoked role keeps receiving its topic for the socket's lifetime (plan-specified; deserves an explicit decision in a later plan) · `modules/opd/realtime.ts` is a pure core but is not pinned by `purity.test.ts` · `GET /opd/visits` does its queue-entry read directly in the controller (one query, module-isolated; worth lifting into `queue.ts` when a task owns it).
