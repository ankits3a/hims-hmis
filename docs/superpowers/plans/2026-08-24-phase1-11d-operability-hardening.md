...
> that NO role holds — **AS MEASURED BEFORE OWNER RULING 7, WHICH FOLLOWS AND CHANGES THESE TWO
> NUMBERS TO 42 AND 17.** Held = 39 of 59 at that moment (`opd.*` 14 and `billing.*` 14 from the two README tables,
...
| V20 | T6 | A heartbeat landing mid-sweep is not overwritten by a false `interface.down` | drop the `lastSeenAt` predicate | ≥15 rounds (a floor): a heartbeat interleaved between the sweep's read and its claim → shipped: status stays `up`, **zero** `interface.down` events — **MEASURED 0/15 at `5491c72` and reproduced 0/15 independently by the gate. REFINEMENT: the shipped test is deliberately WIDER than this clause. It admits a `sweep-won-then-restored` round — the sweep genuinely winning the ordering, appending a TRUE `interface.down`, and the heartbeat restoring it — as CORRECT, and asserts the row is never LEFT down. That outcome never occurred in any of the four 15-round runs, so this clause held as measured; but taken literally it would call correct behaviour a defect, and the test is the better statement. The test also carries an ANTI-VACUITY leg (`heartbeatKeptItUp > 0`) without which a probe that stops racing altogether passes the primary assertion — the gate built exactly that probe and only the anti-vacuity leg caught it.**; mutant: the 14/15 false-down window reappears and is quoted | **P/M** |
