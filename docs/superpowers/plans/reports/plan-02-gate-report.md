...
5. **Two stray files on the owner's Windows filesystem**, outside the repo, awaiting manual removal: `C:\opt\hmis\apps\core\src\kernel\auth\{break-glass.test.ts, totp.test.ts}`. An agent's deletion attempt was denied by the permission system and correctly not retried.
