...
> ## ⚠ CORRECTION — a post-ship audit falsified three claims in this report
>
> **Read [`plan-06-1-audit-findings.md`](plan-06-1-audit-findings.md) before relying on anything below.** Two independent read-only auditors run against `cab41c3` on 2026-08-14/15 found **two CRITICALs, four MODERATEs and eleven MINORs**, and §6 of this report — "a zero-catch run is not proof the gates were sharp; it means they were never seriously tested" — turned out to be the most accurate sentence in it.
>
> 1. **§9's "A DPCO ceiling can no longer depend on heap order" is misleading.** `newId()` is non-monotonic `ulid()`, not `monotonicFactory()`, so T3's `id DESC` tie-break is decided by 80 bits of randomness whenever two rows are minted in the same millisecond. Reproduced end-to-end: **6 of 200 trials billed the superseded ceiling.** The ordering no longer depends on heap order — it depends on ULID randomness instead. (Findings A1.)
> 2. **§9's "An approval can only activate the version it was raised for" is false for one write.** The rejected-approval belt UPDATE in `versions.ts` runs *before* both the approval-subject guard and the SoD guard, so a mis-bound rejected approval drives an unrelated healthy version to terminal `rejected`, and the SoD-forbidden drafter can trigger it. Reproduced. (Findings B1.)
> 3. **§3's 40/40 independent race re-runs did not discriminate.** The runs happened and the numbers are real, but mutants that revert the serializer to its broken Plan-06 shape — or delete it outright — **also pass 10/10 isolated**. Migration 0008's index, not the lock, is what the test observes. The serializer is very likely correct (lens ① proved it by `EXPLAIN` and 150 concurrent activations), but **no test proves it**, and three layers of verification measured something that cannot tell the difference. (Findings A2.)
>
> §11's "the two-audit rule held" needs the same qualification: the derivation audit held, but the **discrimination** audit was itself unexecuted — Assertion Book row A7's claimed kill is refuted by actually running it. A hand-performed discrimination audit is a prediction; only a mutant is evidence.
...
- **Both activation race tests re-run 20× isolated each, by me, after pipeline A** — 40/40 clean, **`11 skipped, 1 passed, 12 total` on every single run** (isolation read from OUTPUT, tripwire 19), zero `40P01` deadlocks in any of the 40 logs. The Plan 06 §7.2 lesson says not to take a gate's word for race evidence; this did not. **⚠ CORRECTION: this evidence does not discriminate.** A mutant that reverts the serializer to its broken Plan-06 shape, and one that deletes it entirely, both pass 10/10 isolated — the index enforces the invariant the test observes. Re-running someone else's non-discriminating test more times produces more non-discriminating evidence. See findings A2.
...
- **An approval can only activate the version it was raised for** — `subjectType` and `subjectId` are both checked at the single consumption site. **Partially retracted:** the guard is correct for activation, but the rejected-approval belt UPDATE fires *before* it, so a mis-bound rejected approval can still write an unrelated version to terminal `rejected` (findings B1). Also, only the `subjectId` half is under test (m7).
...
- ~~**Regulated-price resolution is a total order** — `(effectiveFrom DESC, id DESC)`, so the same-date gazette-correction path resolves to the correction, deterministically, at both the resolver and the list route. **A DPCO ceiling can no longer depend on heap order.**~~ **RETRACTED — see the correction block above.** The order is total but is *not* insertion order: `newId()` is non-monotonic `ulid()`, so same-millisecond rows resolve by coin flip and a superseded ceiling can win (6/200 reproduced). Safe today only because a one-row-per-request path takes longer than a millisecond; any bulk gazette import breaks it. Plan 08 must not assume this.
