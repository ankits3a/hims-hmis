...
| MAJOR | **Ctrl+N was dead where it was needed** — below the typing guard, with an `autoFocus` search box; and Chrome-reserved regardless, so the handler was for a keystroke the browser eats | `b3310f7`, then **ruled** to `Alt+N` global in `250979b` |
...
4. ~~**What chord replaces `Ctrl+N` for "new walk-in"?**~~ — **RULED BY ME, NOT SENT UP (`250979b`), and listing it here was my error.** The owner's standing rule is *decide on industry standard; stop only for money, procurement or law*, and a keyboard chord is none of the three. **`Alt+N`, global.** It needed no invention: `keyboard.tsx` already binds `Alt+M/A/B/D/V/C/P` for global actions and `Alt+N` was free, so the convention the application already shipped chose the letter. It is GLOBAL rather than seat-local for the reason the seat already gives for not rebinding `Ctrl+K` — a shortcut learned on one screen must mean the same thing on the next — so the seat now claims no navigation chord at all. `F2` and the `Register new` button are unchanged.

**Three rulings remain, and they are the right three:** two are money (1 and 3) and one is a product decision the owner is meant to make by looking at both screens (2).
