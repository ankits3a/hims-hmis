 ...
 */
function DeskDock({ log, said, busy, onAsk, onDismiss, draft: agentDraft, plan, payPlan, returnPlan, duplicates, onDraftDone }: {
  ...
  /** PHARMACY P6 (hygiene) — the agent's possible duplicate items ("duplicate items dikhao"), a card that opens the office's items side. */
  duplicates: DuplicateItemsCardData | null;
  onDraftDone: () => void;
}): React.ReactElement {
  ...
  return (
    <div style={{ flexShrink: 0, background: "var(--agent)", color: "var(--agent-fg)" }}>
      ...
      {duplicates === null ? null : <DuplicateItemsCard plan={duplicates} onDone={onDraftDone} />}
      {/* The card IS the answer to a draft; saying it again above the bar is the same sentence twice. */}
      {said === null || (agentDraft !== null && !agentDraft.alreadyOpen) || plan !== null || payPlan !== null || returnPlan !== null || duplicates !== null ? null : (
