...
type OfficeView = "buy" | "pay" | "returns" | "reports" | "controlled" | "items";
...
export function PharmacyOffice({ initialView }: { initialView?: OfficeView } = {}): React.ReactElement {
  ...
  const views = ([
    ...
    ...(canControlled ? ["controlled"] : []), ...(canItems ? ["items"] : []),
  ] as OfficeView[]);
  const [view, setView] = useState<OfficeView>(() => {
    const v = new URLSearchParams(window.location.search).get("view");
    return initialView ?? (v === "pay" || v === "returns" || v === "reports" || v === "controlled" || v === "items" ? v : "buy");
  });
  ...
  return (
    <div className="space-y-5 p-4">
      ...
      {shown === "pay" ? <PayView /> : shown === "returns" ? <ReturnsView /> : shown === "reports" ? <ReportsView /> : shown === "controlled" ? <ControlledView /> : shown === "items" ? <ItemsView /> : (<>
