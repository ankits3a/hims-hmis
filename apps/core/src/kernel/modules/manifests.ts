...
export const ALL_MANIFESTS: readonly ModuleManifest[] = [
  ...
  pharmacyManifest,
  // PLAN 20 T1 — appended. The roster is its own manifest for the reason `aerb` is (D1): every
  // department owes it rows and it reaches into none of them. This task: three permissions, no
  // menu, no subscription, no job — the seam ships inert and T2's resolver is its first reader.
  rosterManifest,
];
