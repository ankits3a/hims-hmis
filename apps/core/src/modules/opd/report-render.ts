...
/**
 * ═══ THE OPD REPORT ON PAPER, AND AS A SPREADSHEET ═══
 *
 ...
 *
 * A sheet says which DAYS it counted, in its title and beside its date. A weekly sheet that did not
 * say "Monday to Saturday" would be a number nobody could reconcile against the month.
 */
...
const PERIOD_TITLE: Record<ReportPeriod, string> = {
  day: "OPD Day Report",
  week: "OPD Weekly Report",
  month: "OPD Monthly Report",
};

const PERIOD_STEM: Record<ReportPeriod, string> = { day: "Day", week: "Week", month: "Month" };
...
export function definitionsFor(period: ReportPeriod): readonly string[] {
  return period === "week" ? [...DEFINITIONS, WEEK_DEFINITION] : DEFINITIONS;
}
...
/** `19-Sep-2026, Saturday` for one day; `21-Sep-2026 to 26-Sep-2026 · 6 days` for a period. */
export function rangeLabel(range: ReportRange): string {
  if (range.from === range.to) return `${dayLabel(range.from)}, ${weekday(range.from)}`;
  const days = daysBetween(range.from, range.to);
  return `${dayLabel(range.from)} to ${dayLabel(range.to)} · ${String(days)} day${days === 1 ? "" : "s"}`;
}
...
/** What a weekly sheet owes the reader when its Sunday was not empty. */
export function excludedSundayLine(excluded: ExcludedSunday | null): string | null {
  if (excluded === null) return null;
  const n = excluded.consulted;
  return `${String(n)} consultation${n === 1 ? "" : "s"} on Sunday ${dayLabel(excluded.date)} ${n === 1 ? "is" : "are"} not included: a week runs Monday to Saturday.`;
}
...
function title(heading: string, sub: string, range: ReportRange, generatedAt: string): string {
  const label = range.from === range.to ? "Date" : "Period";
  return `<div class="ttl"><div><h1>${esc(heading)}</h1><div class="k">${esc(sub)}</div></div>`
    + `<div class="r">${label}: <b class="num">${esc(rangeLabel(range))}</b>`
    + `<div class="num">Generated ${esc(generatedLabel(generatedAt))}</div></div></div>`;
}

function provisionalNote(provisional: boolean, stillOpen: number, period: ReportPeriod): string {
  ...
  const head = provisional
    ? (period === "day" ? "The day is not over." : "This period is not over.")
    : "Visits still open.";
  return `<div class="warn"><b>${head}</b>${open} Figures may change.</div>`;
}

function sundayNote(excluded: ExcludedSunday | null): string {
  const line = excludedSundayLine(excluded);
  return line === null ? "" : `<div class="warn">${esc(line)}</div>`;
}
...
function notes(period: ReportPeriod): string {
  return `<div class="notes">${definitionsFor(period).map((d) => {
...
export function fileStem(range: ReportRange, department?: { code: string }): string {
  ...
  if (department !== undefined) parts.push(department.code);
  parts.push(range.from === range.to ? range.from : `${range.from}-to-${range.to}`);
  return parts.join("-");
}
...
export function renderReport(r: OpdReport): RenderedReport {
  ...
  const docTitle = fileStem(r);
  return { title: docTitle, page: { widthMm: 210, heightMm: 297 }, html: page(docTitle, body) };
}

/** One department's period: the brief, then every patient seen. */
export function renderDepartmentReport(r: OpdDepartmentReport): RenderedReport {
  const d = r.department;
  const manyDays = r.from !== r.to;
  const rows = r.rows.map((p, i) => `<tr><td class="num">${String(i + 1)}</td>`
    + (manyDays ? `<td class="num">${esc(dayLabel(p.date).slice(0, 6))}</td>` : "")
...
function csvHead(
  ...
  excluded: ExcludedSunday | null,
): string[][] {
  const sunday = excludedSundayLine(excluded);
  return [
    [sheetText(hospital.name)],
    ...hospital.addressLines.map((l) => [sheetText(l)]),
    [heading, rangeLabel(range)],
    ["Period", range.from === range.to ? range.from : `${range.from} to ${range.to}`],
    ["Status", provisional ? "Provisional — this period is not over" : "Final"],
    ["Generated", generatedLabel(generatedAt)],
    ...(sunday === null ? [] : [["Note", sunday]]),
    [],
  ];
}
...
export function departmentReportCsvRows(r: OpdDepartmentReport): string[][] {
  const d = r.department;
  return [
    ...
    ...r.rows.map((p, i) => [
      String(i + 1), p.date, p.time, p.visitNo, sheetText(p.name), p.uhid, p.age, p.gender, sheetText(p.shortAddress),
