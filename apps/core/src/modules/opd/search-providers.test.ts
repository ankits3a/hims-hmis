...
describe("OPD search providers", () => {
  ...
  async function run(
    ...
    over: { chips?: SearchChip[]; range?: { from: string; to: string }; limit?: number } = {},
  ): Promise<SearchProviderResult> {
    const limit = over.limit ?? 5;
    const query = { ...parseSearchQuery(text, limit), chips: over.chips ?? [], ...(over.range ? { range: over.range } : {}) };
    return p.run({ db, actor, query, limit, signal: new AbortController().signal });
  }
  ...
  describe("appointments", () => {
    ...
    it("a patient chip needs no text, and a date range narrows it", async () => {
      ...
      expect((await run(appointmentSearchProvider, clerk.actor, "", { chips })).total).toBe(2);

      const ranged = await run(appointmentSearchProvider, clerk.actor, "", { chips, range: { from: "2026-08-01", to: "2026-08-31" } });
      expect(ranged.total).toBe(1);
      expect(ranged.hits[0]?.meta?.date).toBe("2026-08-20");
    });
    ...
    it("a doctor chip alone is a valid query", async () => {
      ...
      expect((await run(appointmentSearchProvider, clerk.actor, "", { chips })).total).toBe(1);
    });
  });
});
