...
describe("the pharmacy event catalog (16c T1)", () => {
  it("is twenty-two events, all module pharmacy, names unique (P2 added the register's two, P6 the return, P19 the walk-in sale's two, P19b its return, PD-5b the resolution, PD-D18 the shelf label, PD-9 the authorisation's two, 2026-09-23 the salt match, P1 the short book's two)", () => {
    expect(PHARMACY_EVENTS).toHaveLength(22); // P1 (parity): +2, short_book.noted/resolved; 2026-09-23: +1, dispense.line_matched; P2: +2, the register of pharmacists; P6: +1, dispense.line_returned; P19: +2; P19b: +1, retail.line_returned; PD-5b: +1, dispense.line_resolved; PD-D18: +1, shelf.location_set
    for (const e of PHARMACY_EVENTS) expect(e.module).toBe("pharmacy");
    expect(new Set(PHARMACY_EVENTS.map((e) => e.name)).size).toBe(22);
