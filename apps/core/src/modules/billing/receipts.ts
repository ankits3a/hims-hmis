 ...
 */
/**
 * ═══ PLAN 15 T7 — THE BODY BELOW WAS EXTRACTED SO IT HAS TWO ENTRY POINTS, NOT TWO COPIES ═══
 *
 * `allocateReceipt` is `withTx` + this function, unchanged in behaviour. `issueInvoice` calls this
 * one directly, INSIDE its own transaction, to settle an invoice from money the hospital already
 * holds — a day-care deposit taken at booking (Plan 15 DD12).
 *
 * **Why that could not be done by calling `allocateReceipt` afterwards.** D2 step 3 refuses to
 * persist an invoice with a remainder unless a credit extension was requested — *"the invariant that
 * stops a counter minting dues silently"*. An invoice settled from a deposit two seconds later is
 * still an unsettled invoice at the moment it is written, so issue-then-allocate is refused, and
 * relaxing that invariant to let it through would open the hole it exists to close. The deposit has
 * to be applied INSIDE the issuing transaction, which means this body has to be reachable from
 * inside one.
 *
 * The lock order is unchanged and still safe from the new caller: INVOICE, then every receipt of the
 * patient in id order, then the target receipt. `issueInvoice` has just INSERTED the invoice in the
 * same transaction, so locking it is free, and it takes no receipt lock of its own.
 */
export async function allocateOnTx(
  tx: Tx,
  actor: Actor,
 ...
 * not an exception: the ledger carries the amount, and settlement state is re-derived from it.
 *
 * The guards, the locks and the three refusals all live in `allocateOnTx` above; this is the
 * public entry point that gives them a transaction of their own.
 */
export async function allocateReceipt(
  ...
  return withTx(db, (tx) => allocateOnTx(tx, actor, input, now));
}
