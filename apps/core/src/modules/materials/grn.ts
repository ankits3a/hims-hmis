...
async function qcContextFor(
  ...
  const ceilingPaisePerBase = reg?.ceilingPaise === null || reg?.ceilingPaise === undefined
    ? null
    : mrpPerBaseUnit(uoms, reg.ceilingPaise, reg.mrpUom ?? uoms.find((u) => u.toBaseMultiplier === 1)?.uom ?? null);
