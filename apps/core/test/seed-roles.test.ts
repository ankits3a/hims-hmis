...
/** PARITY P5 — the TallyPrime export, the accountant's: `owner` and `billing_manager`, neither with a pharmacy-table column. */
...
/**
 * PHARMACY P6 (patient messages) — the Messages side's one grant outside the pharmacy table: the owner, who
 * contracts the provider. The counter's and the in-charge's ticks ARE in the table.
 */
const PATIENT_MESSAGES_README_PROSE =
  "`pharmacy.messages.manage` (the DLT\ntemplate ids, the WhatsApp template names and the pharmacy's phone on the office's Messages side) goes to\n`pharmacy_incharge` and `owner`, who contracts the provider.";
const PATIENT_MESSAGES_PAIRS: readonly string[] = ["owner/pharmacy.messages.manage"];
...
describe("seed:roles — the census pins, stated before anything is compared (§2.49)", () => {
  it("ALL_MANIFESTS declares one hundred and thirty-four permissions, by module", () => {
    ...
    expect(Object.fromEntries(byKey)).toEqual({
      ...
      materials: 22, // PHARMACY P6 item merge: +1, materials.items.merge; parity P4: +3, materials.returns.manage / .approve, materials.writeoffs.manage; parity P3: +4, materials.bills.manage / .accept_difference, materials.payments.prepare / .record; parity P2: +1, materials.po.raise; 14c first slice: +2, materials.counts.manage and .perform
  ...
  it("the role model is thirty-eight roles, three hundred and twenty-four grants, one hundred and forty-two distinct permissions", () => {
    ...
    expect(Object.fromEntries(ROLE_MODEL.map((r) => [r.roleKey, r.permissions.length]))).toEqual({
      ...
      pharmacy: 34, // PHARMACY P6 (patient messages): +1, pharmacy.messages.consent; PHARMACY P6: +2, pharmacy.ndps.custody and .witness; parity P4: +1, materials.returns.manage; parity P3: +1, materials.bills.manage; parity P2: +1, materials.po.raise; P19: +2, pharmacy.retail.sell and patients.register; P20: +1, pharmacy.downtime.enter; transfer screen: +1, materials.stock.receive
      ...
      billing_manager: 22, // parity P5 (Tally): +1, pharmacy.tally.export; parity P5: +1, pharmacy.reports.read (not the margin)
      ...
      medical_superintendent: 20, // PHARMACY P6: +2, pharmacy.licences.manage and pharmacy.ndps.witness; PHASE R (R1): +3, roster manage/publish/read; OPD day report: +1, opd.reports.read; P19: +1, pharmacy.retail.manage; P17: +2, pharmacy.register.read and .read_sealed; staff-reports T0: +1, staff.reports.history.full
      duty_manager: 1,
      ...
      materials_head: 27, // PHARMACY P6 item merge: +1, materials.items.merge; PHARMACY P6: +1, pharmacy.ndps.witness; parity P5: +2, pharmacy.reports.read and .margin; parity P4: +3, materials.returns.manage / .approve, materials.writeoffs.manage; parity P3: +4, materials.bills.manage / .accept_difference, materials.payments.prepare / .record; parity P2: +1, materials.po.raise; approvals spine: +2, approvals.requests.read and .decide — it is the approverRole on materials_near_expiry_acceptance and could not open it
      storekeeper: 7,
      ...
      pharmacy_incharge: 14, // PHARMACY P6 (patient messages): +2, the consent chip and the Messages side; PHARMACY P6: +3, the cabinet's key, the witness and the licences; parity P5: +2, pharmacy.reports.read and .margin; parity P4: +2, materials.writeoffs.manage and materials.recall.manage; parity P3: +2, materials.payments.prepare and .record; PHARMACY P17 — the register read and its unredacted copy; P19: +1, the retail licence
    });
  ...
  it("the reachability census closes: 161 declared = 147 held + 14 not yet modelled", () => {
    ...
    expect(heldPermissions()).toHaveLength(181); // PHARMACY P6 (patient messages): +2 on top of item merge;  PHARMACY P6 item merge: +1, granted where it is declared; PHARMACY P6: +3, granted where they are declared; parity P5 (Tally): +1; parity P5: +2, granted where they are declared; parity P4: +3, granted where they are declared; parity P3: +4, granted where they are declared; parity P2: +1, granted where it is declared; PHASE R (R1): +3, granted where they are declared; OPD day report: +1, granted where it is declared; P20: +1; P19: +2, granted where they are declared; P17: +1, granted where it is declared; 14c: +2, granted where they are declared; P9: +1, granted where it is declared; P2: +1, granted where it is declared; FD-31: +1; FD-30: 148, opd.prescription.draft, granted in the commit that declares it; FD-27: 147, `opd.paper.reprint`, granted in the commit that declares it; 17c owner ruling: 140 (approvals.requests.create); PLAN 18a T2: 111 -> 131; VD-1 T4: 133; RC-2 T4: 134; 18b T1: 135; 16c T1: 139, the four pharmacy.* strings, granted in the commit that declares them; 18c T1: 143; 17-E T1: 144, the three aerb.* strings, likewise granted where they are declared // MERGE 2026-09-15: measured from the failing run
  ...
  it("the README carries exactly four permission tables, of the measured shapes", () => {
    ...
    expect(materialsTable.rowCount).toBe(22); // PHARMACY P6 item merge: +1 row; parity P4: +3 rows; parity P3: +4 rows
    expect(materialsTable.cells.size).toBe(22);
    expect(tablePairs(materialsTable)).toHaveLength(37); // PHARMACY P6 item merge: +1 (items.merge to the head); parity P4: +4 (returns.manage to the head and pharmacy; returns.approve and writeoffs.manage to the head); parity P3: +5 (bills.manage to the head and pharmacy; accept_difference, payments.prepare and .record to the head); parity P2: +2, materials.po.raise to the head and pharmacy; transfer screen: +1, pharmacy/materials.stock.receive
    expect(tablePairs(materialsTable).filter((p) => p.startsWith("materials_head/"))).toHaveLength(22); // PHARMACY P6 item merge: +1
...
describe("seed:roles — README parity, cell for cell (V3)", () => {
  ...
  it("V3: the rulings' seventy-four pairs are exactly the model's non-table rows, with every README prose line quoted", () => {
    ...
    // Pharmacy P6 (patient messages): the owner keeps the Messages side, because the owner contracts the provider.
    expect(readme).toContain(PATIENT_MESSAGES_README_PROSE);
