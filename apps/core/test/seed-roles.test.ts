...
describe("seed:roles — the census pins, stated before anything is compared (§2.49)", () => {
  it("ALL_MANIFESTS declares one hundred and thirty-four permissions, by module", () => {
    ...
    expect(Object.fromEntries(byKey)).toEqual({
      ...
      opd: 19, // FD-31: +`opd.prescription.transcribe` (the desk's Mode B key)
      billing: 14,
      alerts: 0,
      ops: 3,
  ...
  it("the role model is thirty-eight roles, three hundred and twenty-four grants, one hundred and forty-two distinct permissions", () => {
    ...
    expect(Object.fromEntries(ROLE_MODEL.map((r) => [r.roleKey, r.permissions.length]))).toEqual({
      ...
      owner: 15, // P19: +1, pharmacy.retail.manage; P17: +2, pharmacy.register.read and .read_sealed; staff-reports T0: +2, staff.reports.read (a defect closed) and .history.full
      ...
      medical_superintendent: 14, // P19: +1, pharmacy.retail.manage; P17: +2, pharmacy.register.read and .read_sealed; staff-reports T0: +1, staff.reports.history.full
      duty_manager: 1,
  ...
  it("the reachability census closes: 161 declared = 147 held + 14 not yet modelled", () => {
    ...
    expect(heldPermissions().length + NOT_YET_MODELLED.length).toBe(174); // P20: +1; P19: +2; P17: +1; 14c: +2; P9: +1; P2: +1; FD-31: +1; FD-30: +1; FD-27: +1, `opd.paper.reprint`; 18b T1: +1; 16c T1: +4; 18c T1: +3; 17-E T1: +1 // MERGE 2026-09-15: measured from the failing run
  });
...
describe("seed:roles — executed against a database (V5)", () => {
  ...
  it("V5: is idempotent — the second run creates nothing, grants nothing, and still reports the census", async () => {
    ...
    expect(second.roles.map((r) => r.already.length)).toEqual([13, 17, 6, 21, 15, 8, 1, 28, 14, 20, 15, 14, 1, 3, 3, 3, 5, 1, 13, 7, 15, 9, 4, 4, 3, 6, 17, 9, 4, 17, 15, 10, 13, 4, 3, 1, 2, 5, 3]); // transfer screen: INDEX 7 pharmacy 27 -> 28. P19: the first run's four changes, again. P5: INDEX 7 pharmacy 22. P2: INDEX 7 pharmacy 20. FD-27: the twin of the array above — INDEX 0 front_office and INDEX 7 cashier, both +1 for `opd.paper.reprint`. 17c owner ruling: lab_reception 16 -> 17; 18b T1: radiographer 9, modality_bridge 1; 16c T1: pharmacy 19, pharmacy_assistant 5; 17-E T1: INDEX 25, pathologist 16 -> 17
