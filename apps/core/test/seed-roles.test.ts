...
const CASHIER_SEAT_PAIRS: readonly string[] = [
  "cashier/opd.visits.open",
  "cashier/opd.visits.read",
...
describe("seed:roles — the census pins, stated before anything is compared (§2.49)", () => {
  it("ALL_MANIFESTS declares one hundred and thirty-one permissions, by module", () => {
    ...
    expect(installedRegistry().allPermissions()).toHaveLength(157); // RC-1 T2: 146 -> 147; VD-1 T4: 148; RC-2 T4: 149; 18b T1: 150 (radiology.mwl.read); 16c T1: 154, the four pharmacy.* strings; 18c T1: 157, the three aerb.* strings
  });

  it("the role model is thirty-six roles, three hundred and two grants, one hundred and thirty-seven distinct permissions", () => {
    ...
    // FD-25 / owner ruling 2026-09-04: 302 -> 306, the cashier's four. `modelPermissions` and
    // `heldPermissions` do NOT move with it — all four strings already existed and were already
    // held by somebody, so this ruling adds GRANTS and not vocabulary. A ruling that moved the
    // permission census too would be declaring a new power, which this one deliberately is not.
    expect(modelPairs()).toHaveLength(306); // 17c owner ruling: +1, approvals.requests.create to lab_reception; RC-2 T4: +2, the enrol grants; 18b T1: +2 (radiology.mwl.read); 16c T1: +16 (pharmacy +11, pharmacy_assistant +5)
    ...
    expect(modelPermissions()).toHaveLength(137); // 17c owner ruling: +1, approvals.requests.create; RC-2 T4: +1, membership.instrument.enrol; 18b T1: +1, radiology.mwl.read; 16c T1: +4, pharmacy.*
  ...
  it("the reachability census closes: 157 declared = 142 held + 15 not yet modelled", () => {
    ...
    expect(installedRegistry().allPermissions()).toHaveLength(157);
    ...
    expect(heldPermissions()).toHaveLength(143); // 17c owner ruling: 140 (approvals.requests.create); PLAN 18a T2: 111 -> 131; VD-1 T4: 133; RC-2 T4: 134; 18b T1: 135; 16c T1: 139, the four pharmacy.* strings, granted in the commit that declares them; 18c T1: 143, the three aerb.* strings, likewise granted where they are declared
    ...
    expect(heldPermissions().length + NOT_YET_MODELLED.length).toBe(157); // 18b T1: +1; 16c T1: +4; 18c T1: +3
  });
...
describe("seed:roles — README parity, cell for cell (V3)", () => {
  it("V3: the four tables and the model agree over the table-derived subset, both directions", () => {
    ...
    expect(fromReadme).toHaveLength(128 + tablePairs(radiologyTable).length + tablePairs(pharmacyTable).length);
  ...
  it("V3: the rulings' seventy-four pairs are exactly the model's non-table rows, with every README prose line quoted", () => {
    ...
    expect(NON_TABLE_PAIRS).toHaveLength(136); // 17c owner ruling: +1 (lab_reception/approvals.requests.create); 16c T1: +10, PHARMACY_PAIRS
...
describe("seed:roles — executed against a database (V5)", () => {
  ...
  it("V5: is idempotent — the second run creates nothing, grants nothing, and still reports the census", async () => {
    ...
    expect(first.roles.map((r) => r.created)).toEqual(Array(36).fill(true)); // 18b T1: 34; 16c T1: 35; 18c T1: 36
    ...
    // FD-25 — the EIGHTH entry moves, `cashier` 11 -> 15, and nothing else does. In ROLE_MODEL order
    // that is front_office, front_office_supervisor, vitals_desk, doctor, opd_admin, display,
    // pharmacy, THEN cashier — checked by name against the model rather than counted along the row,
    // because this array's own comment records that a bare-integer census gives you no name to check
    // and that somebody has already moved the wrong `2` in it.
    expect(first.roles.map((r) => r.granted.length)).toEqual([12, 16, 6, 20, 8, 1, 19, 15, 20, 10, 10, 1, 2, 3, 3, 5, 1, 11, 6, 15, 9, 4, 4, 3, 6, 16, 8, 4, 17, 15, 10, 13, 4, 3, 1, 5]); // 17c owner ruling: lab_reception 16 -> 17; 18b T1: radiographer 9, modality_bridge 1; 16c T1: pharmacy 8 -> 19, pharmacy_assistant 5; 18c T1: radiologist 14 -> 15, radiographer 9 -> 10, and radiation_safety_officer's 3 inserted after pcpndt_incharge
    expect(first.roles.every((r) => r.already.length === 0)).toBe(true);
    expect(first.declared).toBe(157); // RC-1 T2's flow lock, VD-1 T4's history read, RC-2 T4's enrol, 18b T1's mwl read, 16c T1's four pharmacy.* strings, 18c T1's three aerb.* strings
    ...
    // that declares them.
    expect(first.held).toBe(137); // 17c owner ruling: 133 -> 134 (approvals.requests.create); RC-1 T2 — 125 -> 126, flow lock; VD-1 T4 -> 127; RC-2 T4 -> 128; 18b T1 -> 129; 16c T1 -> 133
    expect(first.held).toBe(modelPermissions().length);
    expect(heldPermissions()).toHaveLength(143); // 17c owner ruling; RC-1 T2, VD-1 T4, RC-2 T4's enrol, 18b T1's mwl read, 16c T1's four pharmacy.* strings, then 18c T1's three aerb.* strings
    ...
    // PLAN 17 T2 / DD16 — 25 -> 29 with the lab's four.
    expect(second.roles.map((r) => r.created)).toEqual(Array(36).fill(false));
    ...
    // FD-25 — the eighth entry again, `cashier` 11 -> 15. This is the SECOND of the two places, and
    // the comment below is why it is called out rather than quietly edited: nothing names this array
    // and no grep finds it from the grant that moved it.
    expect(second.roles.map((r) => r.already.length)).toEqual([12, 16, 6, 20, 8, 1, 19, 15, 20, 10, 10, 1, 2, 3, 3, 5, 1, 11, 6, 15, 9, 4, 4, 3, 6, 16, 8, 4, 17, 15, 10, 13, 4, 3, 1, 5]); // 17c owner ruling: lab_reception 16 -> 17; 18b T1: radiographer 9, modality_bridge 1; 16c T1: pharmacy 19, pharmacy_assistant 5
  ...
  it("reports zero holders per role, because seed:roles mints authority and assigns nobody", async () => {
    ...
    expect(report.problems.join(" ")).toContain("NO USER HOLDS ANY OF THE 36 ROLES");
  });
