...
describe("seed:roles — the census pins, stated before anything is compared (§2.49)", () => {
  it("ALL_MANIFESTS declares one hundred and thirty-four permissions, by module", () => {
    ...
    expect(Object.fromEntries(byKey)).toEqual({
      ...
      materials: 18, // parity P3: +4, materials.bills.manage / .accept_difference, materials.payments.prepare / .record; parity P2: +1, materials.po.raise; 14c first slice: +2, materials.counts.manage and .perform
  ...
  it("the role model is thirty-eight roles, three hundred and twenty-four grants, one hundred and forty-two distinct permissions", () => {
    ...
    expect(Object.fromEntries(ROLE_MODEL.map((r) => [r.roleKey, r.permissions.length]))).toEqual({
      ...
      pharmacy: 30, // parity P3: +1, materials.bills.manage; parity P2: +1, materials.po.raise; P19: +2, pharmacy.retail.sell and patients.register; P20: +1, pharmacy.downtime.enter; transfer screen: +1, materials.stock.receive
      ...
      materials_head: 20, // parity P3: +4, materials.bills.manage / .accept_difference, materials.payments.prepare / .record; parity P2: +1, materials.po.raise; approvals spine: +2, approvals.requests.read and .decide — it is the approverRole on materials_near_expiry_acceptance and could not open it
      storekeeper: 7,
      ...
      pharmacy_incharge: 5, // parity P3: +2, materials.payments.prepare and .record; PHARMACY P17 — the register read and its unredacted copy; P19: +1, the retail licence
    });
  ...
  it("the README carries exactly four permission tables, of the measured shapes", () => {
    ...
    expect(materialsTable.rowCount).toBe(18); // parity P3: +4 rows
    expect(materialsTable.cells.size).toBe(18);
    expect(tablePairs(materialsTable)).toHaveLength(32); // parity P3: +5 (bills.manage to the head and pharmacy; accept_difference, payments.prepare and .record to the head); parity P2: +2, materials.po.raise to the head and pharmacy; transfer screen: +1, pharmacy/materials.stock.receive
    expect(tablePairs(materialsTable).filter((p) => p.startsWith("materials_head/"))).toHaveLength(18);
    expect(tablePairs(materialsTable).filter((p) => p.startsWith("storekeeper/"))).toHaveLength(7);
    expect(tablePairs(materialsTable).filter((p) => p.startsWith("pharmacy/"))).toHaveLength(7);
    ...
    expect(pharmacyTable.rowCount).toBe(12); // parity P3: +2, the in-charge's materials.payments.prepare and .record // P20: +1 // P2: +1, pharmacy.pharmacists.manage; P9: +1, pharmacy.register.read; P17: +1, read_sealed; P19: +2, retail.sell and retail.manage
    expect(pharmacyTable.cells.size).toBe(12);
    expect(tablePairs(pharmacyTable)).toHaveLength(15); // parity P3: +2, the in-charge's two payment ticks; P17: +2, the pharmacist in charge's two ticks; P19: +2; P20: +1
...
describe("seed:roles — README parity, cell for cell (V3)", () => {
  it("V3: the four tables and the model agree over the table-derived subset, both directions", () => {
    ...
    expect(fromReadme).toHaveLength(144 + tablePairs(radiologyTable).length + tablePairs(pharmacyTable).length); // transfer screen: 136 -> 137, pharmacy's materials.stock.receive tick; 14c first slice: 132 -> 136, the four counts ticks in the materials table; 17-E T6: 131 -> 132, lab_technician's lab.instruments.operate tick
