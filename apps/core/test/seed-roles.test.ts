...
describe("seed:roles — the census pins, stated before anything is compared (§2.49)", () => {
  ...
  it("the role model is thirty-eight roles, three hundred and twenty-four grants, one hundred and forty-two distinct permissions", () => {
    ...
    expect(Object.fromEntries(ROLE_MODEL.map((r) => [r.roleKey, r.permissions.length]))).toEqual({
      ...
      pharmacy: 33, // PHARMACY P6: +2, pharmacy.ndps.custody and .witness; parity P4: +1, materials.returns.manage; parity P3: +1, materials.bills.manage; parity P2: +1, materials.po.raise; P19: +2, pharmacy.retail.sell and patients.register; P20: +1, pharmacy.downtime.enter; transfer screen: +1, materials.stock.receive
      ...
      pharmacy_incharge: 12, // PHARMACY P6: +3, the cabinet's key, the witness and the licences; parity P5: +2, pharmacy.reports.read and .margin; parity P4: +2, materials.writeoffs.manage and materials.recall.manage; parity P3: +2, materials.payments.prepare and .record; PHARMACY P17 — the register read and its unredacted copy; P19: +1, the retail licence
    });
  ...
  it("the reachability census closes: 161 declared = 147 held + 14 not yet modelled", () => {
    ...
    expect(heldPermissions()).toHaveLength(179); // PHARMACY P6 item merge: +1, granted where it is declared; PHARMACY P6: +3, granted where they are declared; parity P5 (Tally): +1; parity P5: +2, granted where they are declared; parity P4: +3, granted where they are declared; parity P3: +4, granted where they are declared; parity P2: +1, granted where it is declared; PHASE R (R1): +3, granted where they are declared; OPD day report: +1, granted where it is declared; P20: +1; P19: +2, granted where they are declared; P17: +1, granted where it is declared; 14c: +2, granted where they are declared; P9: +1, granted where it is declared; P2: +1, granted where it is declared; FD-31: +1; FD-30: 148, opd.prescription.draft, granted in the commit that declares it; FD-27: 147, `opd.paper.reprint`, granted in the commit that declares it; 17c owner ruling: 140 (approvals.requests.create); PLAN 18a T2: 111 -> 131; VD-1 T4: 133; RC-2 T4: 134; 18b T1: 135; 16c T1: 139, the four pharmacy.* strings, granted in the commit that declares them; 18c T1: 143; 17-E T1: 144, the three aerb.* strings, likewise granted where they are declared // MERGE 2026-09-15: measured from the failing run
  ...
  it("the README carries exactly four permission tables, of the measured shapes", () => {
    ...
    expect(pharmacyTable.rowCount).toBe(19); // PHARMACY P6: +3, the cabinet's key, the witness and the licences; parity P5: +2, the in-charge's pharmacy.reports.read and .margin; parity P4: +2, the in-charge's materials.writeoffs.manage and materials.recall.manage; parity P3: +2, the in-charge's materials.payments.prepare and .record // P20: +1 // P2: +1, pharmacy.pharmacists.manage; P9: +1, pharmacy.register.read; P17: +1, read_sealed; P19: +2, retail.sell and retail.manage
    expect(pharmacyTable.cells.size).toBe(19); // PHARMACY P6: +3; parity P5: +2
    expect(tablePairs(pharmacyTable)).toHaveLength(24); // PHARMACY P6: +5, pharmacy's custody and witness ticks, the in-charge's custody, witness and licences ticks; parity P5: +2, the in-charge's reports and margin ticks; parity P4: +2, the in-charge's write-off and recall ticks; parity P3: +2, the in-charge's two payment ticks; P17: +2, the pharmacist in charge's two ticks; P19: +2; P20: +1
