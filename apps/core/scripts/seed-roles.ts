...
export const ROLE_MODEL: readonly RoleGrants[] = [
  ...
  // ------------------------------------------------------------------------------------------
  {
    ...
    permissions: [
      ...
      "opd.reports.read",
      /**
       * PLAN 20 T1 — THE ROSTER, to READ. The owner sees who is meant to be on; the owner does not
       * make the rota (the MS does), for the reason the owner does not write prescriptions.
       */
      "roster.read",
  ...
  },
  {
    ...
    permissions: [
      ...
      "opd.reports.read",
      /**
       * PLAN 20 T1 — THE ROSTER. The medical superintendent is the authority a clinical roster
       * answers to, so the MS may draft one, PUBLISH one (the governed act, D3) and read them all.
       * The people who will draft day to day — a unit's senior resident, its head, the nursing
       * superintendent — get their strings with their roles, in phase 20-U U1; until then a roster
       * can exist only because the MS made it, which is the right default for a thing that decides
       * who is woken at 02:00.
       */
